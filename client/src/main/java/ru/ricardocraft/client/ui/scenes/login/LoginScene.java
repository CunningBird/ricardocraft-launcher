...
import ru.ricardocraft.client.service.profiles.Texture;
...
import ru.ricardocraft.client.service.runtime.utils.LauncherUpdater;
    ...
    public void doInit() {
        ...
        if (guiModuleConfig.createAccountURL != null) {
            LookupHelper.<Text>lookup(header, "#createAccount")
                    .setOnMouseClicked((e) -> guiObjectsContainer.openURL(guiModuleConfig.createAccountURL));
        }

        if (guiModuleConfig.forgotPassURL != null) {
            LookupHelper.<Text>lookup(header, "#forgotPass")
                    .setOnMouseClicked((e) -> guiObjectsContainer.openURL(guiModuleConfig.forgotPassURL));
        }
        ...
        processing(new ProfilesRequest(), launchService.getTranslation("runtime.overlay.processing.text.profiles"),
                (profiles) -> {
                    ...
                    contextHelper.runInFxThread(() -> {
                        ...
                        if (guiObjectsContainer.getCurrentScene() instanceof LoginScene loginScene) {
                            loginScene.authFlow.isLoginStarted = false;
                        }
                        guiObjectsContainer.setMainScene(getServerMenuScene());
                    });
                }, null);
    }
