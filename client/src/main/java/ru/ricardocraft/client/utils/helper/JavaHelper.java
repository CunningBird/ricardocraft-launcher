...
public class JavaHelper {
    ...
    public synchronized static List<JavaVersion> findJava() {
        ...
        String[] path = System.getenv("PATH").split(JVMHelper.OS_TYPE == JVMHelper.OS.WINDOWS ? ";" : ":");
        for (String p : path) {
            try {
                Path p1 = Paths.get(p);
                Path javaExecPath = JVMHelper.OS_TYPE == JVMHelper.OS.WINDOWS ? p1.resolve("java.exe") : p1.resolve("java");
    ...
    public static void trySearchJava(List<String> javaPaths, List<JavaVersion> result, Path path) throws IOException {
        ...
        Files.list(path).filter(p -> Files.exists(p.resolve("bin").resolve(JVMHelper.OS_TYPE == JVMHelper.OS.WINDOWS ? "java.exe" : "java"))).forEach(e -> {
