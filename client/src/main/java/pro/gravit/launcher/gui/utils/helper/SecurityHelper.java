...
public final class SecurityHelper {
    ...
    public static byte[] digest(DigestAlgorithm algo, InputStream input) throws IOException {
        byte[] buffer = pro.gravit.launcher.gui.utils.helper.IOHelper.newBuffer();
    ...
    public static byte[] digest(DigestAlgorithm algo, Path file) throws IOException {
        try (InputStream input = pro.gravit.launcher.gui.utils.helper.IOHelper.newInput(file)) {
    ...
    public static byte[] digest(DigestAlgorithm algo, String s) {
        return digest(algo, pro.gravit.launcher.gui.utils.helper.IOHelper.encode(s));
    }


    public static byte[] digest(DigestAlgorithm algo, URL url) throws IOException {
        try (InputStream input = pro.gravit.launcher.gui.utils.helper.IOHelper.newInput(url)) {
            return digest(algo, input);
        }
    }
    ...
    public static MessageDigest newDigest(DigestAlgorithm algo) {
        pro.gravit.launcher.gui.utils.helper.VerifyHelper.verify(algo, a -> a != DigestAlgorithm.PLAIN, "PLAIN digest");
        try {
    ...
    public static String verifyToken(String token) {
        return pro.gravit.launcher.gui.utils.helper.VerifyHelper.verify(token, SecurityHelper::isValidToken, String.format("Invalid token: '%s'", token));
    }
    ...
    public static byte[] encrypt(String seed, byte[] cleartext) throws Exception {
        byte[] rawKey = getAESKey(pro.gravit.launcher.gui.utils.helper.IOHelper.encode(seed));
        return encrypt(rawKey, cleartext);
    }

    public static byte[] encrypt(String seed, String cleartext) throws Exception {
        return encrypt(seed, pro.gravit.launcher.gui.utils.helper.IOHelper.encode(cleartext));
    }
