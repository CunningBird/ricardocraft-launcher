...
public abstract class Command {
    ...
    protected static String parseUsername(String username) throws pro.gravit.launcher.gui.utils.command.CommandException {
        try {
            return VerifyHelper.verifyUsername(username);
        } catch (IllegalArgumentException e) {
            throw new pro.gravit.launcher.gui.utils.command.CommandException(e.getMessage());
        }
    }


    protected static UUID parseUUID(String s) throws pro.gravit.launcher.gui.utils.command.CommandException {
        try {
            return UUID.fromString(s);
        } catch (IllegalArgumentException ignored) {
            throw new pro.gravit.launcher.gui.utils.command.CommandException(String.format("Invalid UUID: '%s'", s));
        }
    }
    ...
    public void invokeSubcommands(String... args) throws Exception {
        ...
        if (command == null) throw new pro.gravit.launcher.gui.utils.command.CommandException(String.format("Unknown sub command: '%s'", args[0]));
    ...
    protected final void verifyArgs(String[] args, int min) throws pro.gravit.launcher.gui.utils.command.CommandException {
        if (args.length < min)
