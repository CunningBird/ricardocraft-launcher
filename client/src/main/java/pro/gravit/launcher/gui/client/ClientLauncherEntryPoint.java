...
import pro.gravit.launcher.gui.base.api.KeyService;
...
import pro.gravit.launcher.gui.client.BasicLauncherEventHandler;
import pro.gravit.launcher.gui.client.ClientLauncherCoreModule;
...
public class ClientLauncherEntryPoint {
    public static pro.gravit.launcher.gui.client.ClientModuleManager modulesManager;
    public static pro.gravit.launcher.gui.client.ClientParams clientParams;

    private static Launch launch;
    private static ClassLoaderControl classLoaderControl;

    private static pro.gravit.launcher.gui.client.ClientParams readParams(SocketAddress address) throws IOException {
        try (Socket socket = IOHelper.newSocket()) {
            ...
            try (HInput input = new HInput(new CipherInputStream(socket.getInputStream(), SecurityHelper.newAESDecryptCipher(SecurityHelper.fromHex(Launcher.getConfig().secretKeyClient))))) {
                ...
                pro.gravit.launcher.gui.client.ClientParams params = Launcher.gsonManager.gson.fromJson(IOHelper.decode(serialized), pro.gravit.launcher.gui.client.ClientParams.class);
    ...
    public static void main(String[] args) {
        ...
        pro.gravit.launcher.gui.client.ClientLauncherMethods.checkClass(ClientLauncherEntryPoint.class);
        try {
            realMain(args);
        } catch (Throwable e) {
            LogHelper.error(e);
    ...
    private static void realMain(String[] args) throws Throwable {
        ...
        pro.gravit.launcher.gui.client.ClientLauncherMethods.initGson(modulesManager);
        modulesManager.invokeEvent(new PreConfigPhase());
        LogHelper.debug("Reading ClientLauncher params");
        pro.gravit.launcher.gui.client.ClientParams params = readParams(new InetSocketAddress("127.0.0.1", Launcher.getConfig().clientPort));
        pro.gravit.launcher.gui.client.ClientLauncherMethods.verifyNoAgent();
        if(params.timestamp > System.currentTimeMillis() || params.timestamp + 30*1000 < System.currentTimeMillis() ) {
            ...
            pro.gravit.launcher.gui.client.ClientLauncherMethods.exitLauncher(-662);
            return;
        }
        ...
        if (params.offlineMode) {
            service = pro.gravit.launcher.gui.client.ClientLauncherMethods.initOffline(modulesManager, params);
