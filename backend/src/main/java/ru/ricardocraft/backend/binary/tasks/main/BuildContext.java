...
public class BuildContext {
    ...
    public void pushDir(Path dir, Path targetDir, Map<String, byte[]> hashMap, boolean hidden) throws IOException {
        IOHelper.walk(dir, new RuntimeDirVisitor(output, hashMap, dir, targetDir), hidden);
    }

    public void pushEncryptedDir(Path dir, Path targetDir, String aesHexKey, Map<String, byte[]> hashMap, boolean hidden) throws IOException {
    ...
    private final static class RuntimeDirVisitor extends SimpleFileVisitor<Path> {
        ...
        private RuntimeDirVisitor(ZipOutputStream output, Map<String, byte[]> hashs, Path sourceDir, Path targetDir) {
            this.output = output;
            this.hashs = hashs;
    ...
    private final static class EncryptedRuntimeDirVisitor extends SimpleFileVisitor<Path> {
        ...
        private EncryptedRuntimeDirVisitor(ZipOutputStream output, String aesKey, Map<String, byte[]> hashs, Path sourceDir, Path targetDir) {
            this.output = output;
            this.hashs = hashs;
            this.sourceDir = sourceDir;
            this.targetDir = targetDir;
