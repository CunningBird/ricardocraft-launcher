...
public class TokenCommand extends Command {
    ...
    @Autowired
    public TokenCommand(LaunchServerConfig config, AuthManager authManager, KeyAgreementManager keyAgreementManager) {
        super();
        ...
        this.childCommands.put("server", new SubCommand("[profileName] (authId) (public only)", "generate new server token") {
            ...
            public void invoke(String... args) {
                AuthProviderPair pair = args.length > 1 ? config.getAuthProviderPair(args[1]) : config.getAuthProviderPair();
                boolean publicOnly = args.length <= 2 || Boolean.parseBoolean(args[2]);
                ClientProfile profile = null;
                for (ClientProfile p : config.profileProvider.getProfiles()) {
