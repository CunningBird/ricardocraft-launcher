...
import ru.ricardocraft.backend.service.auth.QueryBuilder;
...
public class OpenIDAuthCoreProvider extends AuthCoreProvider {
    ...
    private transient KeyAgreementService keyAgreementService;
...
//                .build();
    ...
    public AuthService.AuthReport refreshAccessToken(String oldRefreshToken, AuthController.AuthContext context) throws JsonProcessingException {
        ...
        return AuthService.AuthReport.ofOAuth(accessToken, refreshToken,
                expiresIn, session);
    }
    ...
    public AuthService.AuthReport authorize(String login, AuthController.AuthContext context, AuthPassword password, boolean minecraftAccess) throws IOException {
        ...
        if (minecraftAccess) {
            ...
            return AuthService.AuthReport.ofOAuthWithMinecraft(minecraftToken, accessToken, refreshToken,
                    expiresIn, session);
        } else {
            return AuthService.AuthReport.ofOAuth(accessToken, refreshToken,
                    expiresIn, session);
